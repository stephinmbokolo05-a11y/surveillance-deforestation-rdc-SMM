...
# Initialisation GEE adaptative (Local / Streamlit Cloud)
@st.cache_resource
def init_ee():
    try:
...
WORK_DIR = r"D:\PROJET_SUIVI_DEFORESTATION_RDC"
...
if is_national:
    ...
    geo_payload = gdf_selected.__geo_interface__ if gdf_selected is not None else None
    scale_res = 1000
else:
    gdf_selected = gdf_provinces[gdf_provinces['NAME_1'] == current_prov] if gdf_provinces is not None else None
    if gdf_selected is not None:
...
# Chargement centralisé des données GEE
if geo_payload is not None:
    ...
    # --- ONGLET 3: MODÉLISATION PROSPECTIVE (2025-2035) ---
        ...
        st.warning(f"💡 **Projection en 2035** : Sans mesure corrective, la zone risk de perdre **{perte_annuelle_moyenne * 11:,.0f} hectares** supplémentaires de forêt d'ici 2035.")
...
"""
    ...
    st.info("Données spatiales en cours de chargement...")
